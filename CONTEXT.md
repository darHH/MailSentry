...
- ✅ **Phase 3 Gmail integration code-complete** — `content.js`: all Gmail selectors isolated in one `SELECTORS` + `get*` helper "BRITTLE ZONE" block; orchestrator runs all 5 checks → `risk.compositeScore` (async for Safe Browsing); Shadow-DOM banner (red ≥0.3 / green) with `risk %`, `<details>` per-signal breakdown table, and Verify button (matches sender to vendor by exact/lookalike/display-name → surfaces on-file phone). SPA-aware: `MutationObserver` + `hashchange`, debounced 350ms, dedupes by sender+subject. Orchestration verified offline (attack→red 70%, clean→green 0%).
