...
- ⬜ **API keys still not acquired** (USER tasks — Safe Browsing + optional OpenAI). Pipeline runs stub-first (link/QR return 0 until a key is entered).
...
- ⬜ **Phase 3 (Gmail integration) not started** — `content.js` still empty. This is the brittle DOM part.
...
**Next step:** Start **Phase 3 — Gmail integration** (the brittle part — isolate it). Build `content.js`: (1) DOM parser behind small helper fns (sender/domain, subject, body text, body links, attachment indicators, `<img>` for QR); (2) orchestrator runs all 5 checks → `risk.compositeScore`; (3) banner injection in a Shadow DOM (decision #1) — red ≥0.3 / green, score + per-signal breakdown tooltip; (4) one-click Verify → vendor lookup → on-file phone. Keep all selectors in one place so a Gmail update breaks only this file.
