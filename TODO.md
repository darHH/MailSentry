...
- [ ] **Wire up real jsQR decode.** *(Deferred post-hackathon 2026-06-26.)* QR scanning stays stubbed for the demo. Reasoning: (1) crafting a working test email is non-trivial — needs an inline `data:` URL to avoid CORS-tainted canvases, which Gmail's compose UI strips, so it requires SMTP scripting + Gmail App Password setup; (2) the demo beat ("Safe Browsing caught this") is fully covered by the regular link path, which already works post-bugfix — pasting a Safe Browsing test URL as a normal link in Gmail produces the exact same red-banner moment without any QR setup; (3) hackathon time is better spent on Phase 5 demo prep. When picked up post-hackathon: drop `jsQR.js` into `utils/`, register it in the manifest before `qrExtractor.js`, and the existing canvas + URL-extract + `linkScore` pipeline takes over. ~50KB, MIT, no deps. CORS-tainted images are already handled (silently skipped); a background-script CORS workaround would be needed for real-world coverage and would require a privacy-policy update.
